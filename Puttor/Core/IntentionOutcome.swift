...
//  intention — pace, line or situation — and for each option: how often it
//  dropped, how often it was played as meant and what that did for the make
//  rate, which way the misses went, how far they finished, and whether a
//  breaking miss stayed on the high side.
//
...
struct IntentionOutcome: Identifiable {
    ...
    static func outcomes(in putts: [Putt], by part: IntentionPart) -> [IntentionOutcome] {
        ...
        for putt in putts where putt.puttNumber > 0 {
            ...
            if !holed {
                ...
                if let leave = leaves.leave(after: putt) {
                    outcome.leaves.append(leave)
                }
            }
